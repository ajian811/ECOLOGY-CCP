...
public class Z_CCP_DELIVERY_DG_SHIPPING extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		try {
			...
			if (rs3.execute(mainSql)) {
				if (rs3.next()) {
					shippings = rs3.getString("shippings").split(",");
				}
			}
			log.writeLog(Arrays.asList(shippings));
			...
			for (int i = 0; i < list.size(); i++) {
				if ("1".equals(list.get(i).get("type"))) {
					...
					rs.executeSql(sql1);
					while (rs.next()) {
						String DELIVERYNO = rs.getString("DELIVERYNO");// 交运单号
						String DELIVERYITEM = rs.getString("DELIVERYITEM");// 交运单项次
						String checkSql = "select * from UF_SPGHSR where 1=1";
						checkSql += " and DELIVERYNO = '" + DELIVERYNO + "'";
						checkSql += " and DELIVERYITEM = '" + DELIVERYITEM + "'";
						log.writeLog("检查的sql:" + checkSql);
						rs1.executeSql(checkSql);
						if (!rs1.next()) {
							log.writeLog("shipadviceno" + list.get(i).get("shipadviceno"));
							log.writeLog("DELIVERYNO" + DELIVERYNO);
							log.writeLog("DELIVERYITEM" + DELIVERYITEM);
							log.writeLog("数据库不匹配！");
							wrong.append(list.get(i).get("shipadviceno") + "\r\n");
							flag = false;
							break;
						}
					}
			...
			if (wrong.length() > 0) {
				wrong.insert(0, "这些shipadviceno必须先作废再能提交：\r\n");
			}
			if (!flag) {
				request.getRequestManager().setMessagecontent(wrong.toString());
			...
			for (int i = 0; i < list.size(); i++) {
				if ("-1".equals(list.get(i).get("type"))) {
					...
					changeShipping(requestid, list.get(i).get("shipadviceno"), tablename, workflowId, "0");
				} else if ("0".equals(list.get(i).get("type"))) {
					String delSql = "delete from UF_SPGHSR where shipadviceno = '" + list.get(i).get("shipadviceno")
							+ "'";
					rs.executeSql(delSql);
					changeShipping(requestid, list.get(i).get("shipadviceno"), tablename, workflowId, "0");
				} else if ("1".equals(list.get(i).get("type"))) {
					changeShipping(requestid, list.get(i).get("shipadviceno"), tablename, workflowId, "1");
				}
			}
	...
	public void changeShipping(int requestid, String shipadviceno, String tablename, String workflowId, String lx) {
		JCO.Client sapconnection = null;
		BaseBean log = new BaseBean();
		try {
			...
			while (rs.next()) {
				...
				CLOSEDATE = Util.null2String(rs.getString("CLOSEDATE"));// 预计结关日期
...
/*
						...
						if ("1".equals(lx)) {
							...
							// 交运项次
							buffer.append("ETD").append("='").append(ETD).append("',");// 预计开航日ETD
							buffer.append("SALEORDER").append("='").append(SALEORDER).append("',");// 销售订单号
							buffer.append("ORDERITEM").append("='").append(ORDERITEM).append("',");// 销售订单项次
							buffer.append("STOCKNO").append("='").append(STOCKNO).append("',");// 物料号
							buffer.append("STOCKDESC").append("='").append(STOCKDESC).append("',");// 物料描述
							buffer.append("LOCATION").append("='").append(LOCATION).append("',");// 库位
							buffer.append("SHIPNUM").append("='").append(SHIPNUM).append("',");// 出货数量
							buffer.append("SALEUNIT").append("='").append(SALEUNIT).append("',");// 出货单位
							buffer.append("SHIPTO").append("='").append(SHIPTO).append("',");// 送达方名称
							buffer.append("SHIPTOADDR").append("='").append(SHIPTOADDR).append("',");// 送达方地址
							buffer.append("REALSHIPNUM").append("='").append(REALSHIPNUM).append("',");// 实际出货数量
							buffer.append("COSTCENTER").append("='").append(COSTCENTER).append("',");// 成本中心
							buffer.append("REMARK").append("='").append(REMARK).append("',");// 备注
							buffer.append("PROCATEGORY").append("='").append(PROCATEGORY).append("',");// 产品大类
							buffer.append("LCEN").append("='").append(LCEN).append("',");// 利润中心
							buffer.append("SHIPM").append("='").append(SHIPM).append("',");// ShippingMark
							buffer.append("SPECNEED").append("='").append(SPECNEED).append("',");// 特殊需求
							buffer.append("MATERDES").append("='").append(MATERDES).append("',");// 客户物料描述
							buffer.append("CUSORDNO").append("='").append(CUSORDNO).append("',");// 客户订单编号
							buffer.append("ORDERADVICENO").append("='").append(ORDERADVICENO).append("',");// OrderAdvice编号
							buffer.append("SOLDTO").append("='").append(SOLDTO).append("',");// 售达方名称
							buffer.append("SOLDTOADDR").append("='").append(SOLDTOADDR).append("',");// 售达方地址
							// buffer.append("AUART").append("='").append(AUART).append("',");//
							// 销售凭证类型
							buffer.append("BSTNK").append("='").append(BSTNK).append("',");// 客户采购订单编号
							...
							// 散装固定包装(包装别)
							buffer.append("WADAT").append("='").append(WADAT).append("',");// 计划货物移动日期
							buffer.append("SPECS").append("='").append(SPECS).append("',");// 规格
							// buffer.append("CHARG").append("='").append(CHARG).append("',");//
							// 批号
							// buffer.append("PACK").append("='").append(PACK).append("',");//
							// 包装性质
							buffer.append("AESKD").append("='").append(AESKD).append("',");// 客户工程修改状态
							// buffer.append("VSTEL").append("='").append(VSTEL).append("',");//
							// 装运点/接收点
							buffer.append("LPRIO").append("='").append(LPRIO).append("',");// 交货优先级
							buffer.append("SDABW").append("='").append(SDABW).append("',");// 特殊处理标记
							buffer.append("NTGEW").append("='").append(NTGEW).append("',");// 净重量
							// buffer.append("SGTXT").append("='").append(SGTXT).append("',");//
							// 项目文本
							buffer.append("TKNUM").append("='").append(TKNUM).append("',");// 装运编号
							buffer.append("EXTI1").append("='").append(EXTI1).append("',");// 外部识别
																							// //
																							// 1
							buffer.append("SIGNI").append("='").append(SIGNI).append("',");// 容器
																							// //
																							// ID
							buffer.append("TNDR_TRKID").append("='").append(TNDR_TRKID).append("',");// 货运代理追踪标识
							buffer.append("ADD01").append("='").append(ADD01).append("',");// 供应量
																							// //
																							// 1
							buffer.append("ADD02").append("='").append(ADD02).append("',");// 供应量
																							// //
																							// 2
							buffer.append("ADD03").append("='").append(ADD03).append("',");// 供应量
																							// //
																							// 3
							buffer.append("TEXT1").append("='").append(TEXT1).append("',");// 附加文本
																							// //
																							// 1
							buffer.append("NAME1").append("='").append(NAME1).append("',");// 名称
																							// //
																							// 1
							buffer.append("BSTKD").append("='").append(BSTKD).append("',");// 客户采购订单编号
							buffer.append("KOSTK").append("='").append(KOSTK).append("',");// 全部拣配/堆放状态
							buffer.append("FERTH").append("='").append(FERTH).append("',");// 生产/检验备忘录
							buffer.append("SPART").append("='").append(SPART).append("',");// 产品组
							buffer.append("ORMNG").append("='").append(ORMNG).append("',");// 装运通知/内向交货的原始数量
							buffer.append("ZGMZBH").append("='").append(ZGMZBH).append("',");// 购买证编号
							buffer.append("ZADDFLAG").append("='").append(ZADDFLAG).append("',");// 是否为苏州大市（X=是）
							buffer.append("ZYSZBH").append("='").append(ZYSZBH).append("',");// 运输证编号
							buffer.append("ZMENGE1").append("='").append(ZMENGE1).append("',");// 采购订单数量1
							buffer.append("ZMENGE2").append("='").append(ZMENGE2).append("',");// 采购订单数量2
							buffer.append("ZMEINS1").append("='").append(ZMEINS1).append("',");// 采购订单计量单位1
							buffer.append("ZMEINS2").append("='").append(ZMEINS2).append("',");// 采购订单计量单位2
							buffer.append("ZMATNR").append("='").append(ZMATNR).append("',");// 物料编号
							buffer.append("ZKUNNR").append("='").append(ZKUNNR).append("',");// 运输公司编号
							buffer.append("ZKUNNRTXT").append("='").append(ZKUNNRTXT).append("',");// 运输公司名称
							buffer.append("ZDATELOW1").append("='").append(ZDATELOW1).append("',");// 可用（可供货的）开始日期
							buffer.append("ZDATEHIGH1").append("='").append(ZDATEHIGH1).append("',");// 可用(可供货的)截至日期
							...
							buffer.append("ZCARS").append("='").append(ZCARS).append("',");// 车号,逗号区分
							...
							buffer.append("UNTTO").append("='").append(UNTTO).append("',");// 交货不足限度
																							// //
																							// 5%
							buffer.append("ZCHARG").append("='").append(ZCHARG).append("',");// 批次备注
							buffer.append("ZDEL").append("='").append(ZDEL).append("',");// 删除
							buffer.append("REMARK1").append("='").append(REMARK1).append("',");// beizhu1
							buffer.append("REMARK2").append("='").append(REMARK2).append("',");// beizhu2
							buffer.append("REMARK3").append("='").append(REMARK3).append("',");// beizhu3
							buffer.append("COMCODE").append("='").append(COMCODE).append("',");// 公司代码
							buffer.append("CONTAINTYPE").append("='").append(CONTAINTYPE).append("',");// 货柜预定类型
							buffer.append("CONTAINQUAN").append("='").append(CONTAINQUAN).append("',");// 柜数
							// buffer.append("MSEHT").append("='").append(MSEHT).append("',");//
							// 度量单位文本(最多10个字符)
							// buffer.append("ZWEIGHT").append("='").append(ZWEIGHT).append("',");//
							// 包材重量
							buffer.append("ETA").append("='").append(ETA).append("',");// ETA
							buffer.append("EREQUESTID").append("='").append(EREQUESTID).append("',");// EREQUESTID
							buffer.append("CLOSEDATE").append("='").append(CLOSEDATE).append("'");// EREQUESTID
							buffer.append(" where 1 =1 ");
							buffer.append(" and DELIVERYNO").append("='").append(DELIVERYNO).append("'");
							buffer.append(" and DELIVERYITEM").append("='").append(DELIVERYITEM).append("'");
