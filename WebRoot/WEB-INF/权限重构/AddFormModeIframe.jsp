...
<table id="topTitle" cellpadding="0" cellspacing="0">
	...
	<td class="rightSearchSpan" style="text-align:right;">
	     <%
			sql = "select id,expendname,hreftarget,opentype from mode_pageexpand where modeid="+modeId+" and issystem=0 and isquickbutton=1 and isshow=1 and ( issystemflag ='' or issystemflag is null)";
			rs.executeSql(sql);
			while(rs.next()){
		...
		<%} %>
		<%
			sql = "select id,expendname,issystem,issystemflag,isquickbutton,isbatch from mode_pageexpand where modeid="+modeId+" and issystem=1 and isshow=1 and showtype=2 and isbatch in(0,2) ";
			rs.executeSql(sql);
			//================
			String billidV;
...
function windowOpenOnSelfCallBack(detailid){
	var url = eval("url_id_"+detailid);
   	...
   	if("<%=isfromTab%>"=="1"){
   		parentWin.location.href = url;//对中文的处理放在FieldInfo里处理
	}else{
