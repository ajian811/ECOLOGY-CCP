...
//sqlwhereparam = URLDecoder.decode(sqlwhereparam,"UTF-8");
...
for(int i=0;i<tempquerystrings.length;i++){
	...
	if(tmpqsArray.length>1){
		if(tmpqsArray[0].toLowerCase().equals("sqlwhere")){
			sqlwhereparam = URLDecoder.decode(xssUtil.get(tmpqsArray[1]),"UTF-8");
...
//sqlcondition = xssUtil.get(sqlcondition);
...
if(!sqlcondition.equals("")){
	sqlcondition = FormModeBrowserSqlwhere.decode(sqlcondition);
...
<%}
else if(htmltype.equals("3") && (type.equals("141")||type.equals("56")||type.equals("27")||type.equals("118")||type.equals("64")||type.equals("137"))){//浏览框
...
<div>
<!-- 显示查询结果 start -->
<div id="splitPageContiner" style="overflow:scroll;width:570px" >  
<%
//查询参数
String formmodeid="0";
...
widthPre = bg3.setScale(4, BigDecimal.ROUND_HALF_UP).doubleValue();
        ...
        tableString+= " sqlform=\""+Util.toHtmlForSplitPage(fromSql)+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlwhere)+"\"  sqlorderby=\""+orderby+"\"  sqlprimarykey=\""+sqlprimarykey+"\" sqlsortway=\"Desc\" poolname=\""+vdatasource+"\" />"+
                              "			<head>";
                 tableString+="				<col width=\"10%\" text=\"模块ID\" hide=\"true\" column=\"formmodeid\" />";
		RecordSet.beforFirst();
		while (RecordSet.next()) {
			...
			if(RecordSet.getString("id").equals("-1")){
				...
				//创建日期
				tableString+="				<col width=\""+temocolwidth+"%\" text=\""+SystemEnv.getHtmlLabelName(722,user.getLanguage())+"\" column=\"modedatacreatedate\" "+orderkey+" otherpara=\"column:modedatacreatetime\" transmethod=\"weaver.formmode.search.FormModeTransMethod.getSearchResultCreateTime\" />";
				...
				label = SystemEnv.getHtmlLabelName(Util.getIntValue(label),user.getLanguage());
	 			tableString+="			    <col width=\""+temocolwidth+"%\" text=\""+label+"\"  column=\""+name+"\"  otherpara=\""+para3+"\" "+orderkey+"  transmethod=\"weaver.formmode.search.FormModeTransMethod.getBrowserOthers\"/>";
			}
		}
...
function BrowseTable_onclick(e){
   ...
   if(target.nodeName =="TD"){
	...
	if( target.nodeName =="TD"||target.nodeName =="A"  ){
		...
		<%}else{%>
			var returnjson = {
				id:jQuery(jQuery(target).parents("tr")[0].cells[0]).find("input[type='checkbox']").val(),
