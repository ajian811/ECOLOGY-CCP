...
if(isbill == 1 && VirtualFormHandler.isVirtualForm(formid)){
	...
	if(src.equals("del")){
		...
		<script language="javascript">
			...
			<%}else{%>
			var url="/formmode/view/AddFormMode.jsp?isfromTab=0&modeId=<%=formmodeid%>&formId=<%=formid%>&type=0&billid=<%=id%>&viewfrom=<%=viewfrom%>&opentype=<%=opentype%>&customid=<%=customid%>&isopenbyself=<%=isopenbyself%>&istabinline=<%=istabinline%>&tabcount=<%=tabcount%>";
...
if(RecordSet2.next()){	
	  ...
	  supvalue += RecordSet2.getString("id")+"_";
}
...
if(RecordSet2.next()){
	  ...
	  if(RecordSet2.next()){			
		  supvalue += RecordSet2.getString(tempfield);
	  }
}else{
	  supvalue = "";
}  
...
%>
    <script type="text/javascript">  
         if(window.parent.frames['leftframe']){
             window.parent.frames['leftframe'].refreshTree("<%=supvalue%>","<%=lastnodeid%>");
         }else if(window.parent.parent.frames['leftframe']){
             window.parent.parent.frames['leftframe'].refreshTree("<%=supvalue%>","<%=lastnodeid%>");
         }
    </script>
<%
	//编辑的时候，修改建模主字段权限
...
<%
	...
	String topage = "/formmode/view/AddFormModeIframe.jsp?fromFlowDoc=1&isfromTab="+isfromTab+"&modeId="+formmodeid+"&formId="+formid+"&type=2&billid="+billid+"&opentype=0&customid="+customid+"&adddocfieldid="+adddocfieldid+"&isdialog="+isDialog+"&isclose=1&templateid="+templateid+"&istabinline="+istabinline+"&tabcount="+tabcount;
...
%>
	<script language="javascript">
		...
		try{
			if(parent && parent.window.opener && parent.window.opener._table){
				parent.window.opener._table.reLoad();
				parent.window.opener.parent.parent.frames['leftframe'].refreshTree("<%=supvalue%>","<%=lastnodeid%>");
			}
			if(parent && parent.parent && parent.parent.window.opener && parent.parent.window.opener._table){
				parent.parent.window.opener._table.reLoad();	
				parent.parent.window.opener.parent.parent.frames['leftframe'].refreshTree("<%=supvalue%>","<%=lastnodeid%>");			
			}
		}catch(e){
		...
		if(subnew==1){
			...
			%>
			window.location.href="/formmode/view/AddFormModeIframe.jsp?modeId=<%=formmodeid%>&formId=<%=formid%>&type=1<%=addurl %>&mainid=<%=mainid%>&istabinline=<%=istabinline%>&tabcount=<%=tabcount%>";
			<%
		}else{
			%>
			var url="/formmode/view/AddFormMode.jsp?isfromTab=0&modeId=<%=formmodeid%>&formId=<%=formid%>&type=0&billid=<%=billid%>&iscreate=<%=iscreate%>&messageid=<%=messageid%>&viewfrom=<%=viewfrom%>&opentype=<%=opentype%>&customid=<%=customid%>&isopenbyself=<%=isopenbyself%>&isdialog=<%=isDialog%>&isclose=1&templateid=<%=templateid%>&mainid=<%=mainid%>&istabinline=<%=istabinline%>&tabcount=<%=tabcount%>";
			if(<%=istabinline%>==1){
				if(<%=tabcount%>==0){
					<%if(billid > 0 && iscreate.equals("1")){%>
						window.parent.location.href=window.parent.location.href;
				...
				if(isfromTab==1){
					...
					if(parent.parent.location.href.indexOf("/formmode/view/ViewMode.jsp")!=-1){
