...
说明：<br/>
1.当此处字段对应值不填写，为空时，对应的字段值在前台执行批量修改后，将被置为空。<br/>
2.当操作为变量时支持以下变量：<br/>
  当前用户$UserId$<br/>
  当前用户所在部门$DepartmentId$<br/>
  当前用户所在部门（包含下级部门）$AllDepartmentId$<br/>
  当前用户所在分部$SubcompanyId$<br/>
  当前用户所在分部（包含下级分部）$AllSubcompanyId$<br/>
  当前日期$date$<br/>
  取卡中某字段的值格式为：$字段列名$<br/>
 ...
 //var dialog = parent.parent.parent.getDialog(parent);
	...
	function setDefaultValue(){
		...
		if(message.length>0){
			...
			message+=" 的值为空，执行此批量修改时，字段的值将会被清空，请确认是否提交？";
		}
		return message;
	}
