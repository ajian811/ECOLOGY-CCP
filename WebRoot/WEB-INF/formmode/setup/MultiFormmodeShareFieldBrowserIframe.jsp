...
String fieldsql = "select b.indexdesc,a.id,a.fieldname,a.fieldhtmltype,a.type,a.detailtable from workflow_billfield a,HtmlLabelIndex b "; 
fieldsql += "where billid = "+formid+" and a.fieldlabel = b.id  "; 
fieldsql += "and a.fieldhtmltype = 3 ";
if(type==1){//人员
	fieldsql += " and a.type in (166,165,17,1) ";	
	if(isRoleLimited.equals("1")){
		fieldsql += " union all select '创建人' as indexdesc,-101 as id,'modedatacreater' as fieldname,'3' as fieldhtmltype,-1 as type,'' as detailtable from workflow_billfield where id="+minbillid;
	}
}else if(type==2){//部门
	fieldsql += " and a.type in (168,167,57,4) ";
	if(isRoleLimited.equals("1")){
		fieldsql += " union all select '创建人部门' as indexdesc,-102 as id,'modedatacreaterDept' as fieldname,'3' as fieldhtmltype,-1 as type,'' as detailtable from workflow_billfield where id="+minbillid;
	}
}else if(type==3){//分部
	fieldsql += " and a.type in (170,169,164,194) ";
	if(isRoleLimited.equals("1")){
		fieldsql += " union all select '创建人分部' as indexdesc,-103 as id,'modedatacreaterSubc' as fieldname,'3' as fieldhtmltype,-1 as type,'' as detailtable from workflow_billfield where id="+minbillid;
	}
}else if(type==4){//岗位
