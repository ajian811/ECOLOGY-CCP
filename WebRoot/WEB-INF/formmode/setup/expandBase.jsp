				...
				<brow:browser viewType="0" id="hrefid" name="hrefid" browserValue='<%= ""+hrefid %>' 
						...
						<span id="batchmodeifyspan" style="color:red; display: <%=batchmodeifyDisplay%>;">
							<%
								...
								if(addstr.indexOf("此处")>-1){
									addstr = addstr.replace("此处", "<a href=\"javascript:addfeild();\">此处</a>");
								}else if(addstr.indexOf("此處")>-1){
									addstr = addstr.replace("此處", "<a href=\"javascript:addfeild();\">此處</a>");
								}else if(addstr.indexOf("here")>-1){
									addstr = addstr.replace("here", "<a href=\"javascript:addfeild();\">here</a>");
								}
							%>
							<%=addstr%>
						</span>
		...
		<tr>
			...
			<td class="e8_tblForm_field" valign="top">
				...
				<!-- 
					参数说明：
						1：$parentid$，卡片数据id
						2：$字段名称$，卡片对应主表中字段名称，例如：$name$&
						3：$field+字段id$，卡片对应主表字段id，例如：$field6704$
				 -->
