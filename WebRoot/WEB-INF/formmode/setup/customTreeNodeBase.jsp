		...
		<table class="e8_tblForm">
				...
				<td class=e8_tblForm_field>
					...
					<table>
						<tr>
							<td>
								<!-- 
									提示：可使用json数组格式输入多个url
									1.格式：
									2.其中id、url、sqhwhere为必须项
									3.id为0表示为不满足其他条件时的默认地址，此项可不输入sqlwhere
								 -->
			...
			<tr id="iscontainssubTr">
				...
				<td class=e8_tblForm_field>
					...
					<!--  
						用在链接目标关联字段过滤查询列表时，过滤出的查询列表数据不仅包含自身还要包含自身的所有下级的值。(仅在链接目标地址为查询列表，且链接目标关联字段不能为空的情况下生效)
					 -->
				</td>
			</tr>
