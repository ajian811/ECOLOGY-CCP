...
<%
	...
	if(src.equalsIgnoreCase("getRelateTableInfo")){//获取引用数据库表
		...
		String sql = "select * from modeDataInputmain where entryID="+entryID;
	  	...
	  	if(rs.next()){
	  		datasourcename = Util.null2String(rs.getString("datasource"));
	  	}
	  	String formid = "";
		...
		while(rs.next()){
			...
			if(datasourcename.trim().equals("")){
                ...
                if(_billid!=0){
                    rs4.executeSql("select namelabel from workflow_bill where id="+_billid);
                    if(rs4.next()){
                    	String tablenamelabel = SystemEnv.getHtmlLabelName(rs4.getInt("namelabel"),user.getLanguage());
                    	tablenamespan += tablenamelabel;
                    }
                    if(_isdetail!=1){
                    	tablenamespan += "("+SystemEnv.getHtmlLabelName(21778,user.getLanguage())+")";
