...
<FORM NAME=SearchForm STYLE="margin-bottom:0" action="triggerTableFieldsBrowserIframe.jsp" method=post>
			...
			<% 
				String tableString=""+
				   ...
				   "<head>"+							 
						 ...
						 "<col width=\"0%\" hide=\"true\"  transmethod=\"weaver.formmode.search.FormBrowserTransMethod.forHtml\" text=\"\" column=\"tablename\"/>"+
						 "<col width=\"0%\" hide=\"true\"  transmethod=\"weaver.formmode.search.FormBrowserTransMethod.forHtml\" text=\"\" column=\"tabfix\"/>"+
				   "</head>"+
				   "</table>";
			%>
...
function afterDoWhenLoaded(){
	jQuery(".ListStyle").children("tbody").find("tr[class!='Spacing']").bind("click",function(){
		...
		var returnjson = {id:$(this).find("td:eq(1)").text(),name:tabletype+$(this).find("td:eq(2)").text(),other1:$(this).find("td:eq(4)").text()};
		//var returnjson = {id:$(this).find("td:first").next().text(),name:$(this).find("td:first").next().next().text(),other1:$(this).find("td:first").next().next().next().next().text()};
		if(dialog){
