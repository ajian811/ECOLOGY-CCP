...
<head>
	...
	<script type="text/javascript">
		...
		function createApp(superFieldId){
			...
			appDlg.Height = 360;
			var url = "/formmode/setup/addDefineApp.jsp?superFieldId="+superFieldId+"&flag=create";
			if(jQuery("#subCompanyId").length>0){
				var subCompanyId = jQuery("#subCompanyId").val();
				if(subCompanyId!=""){
					url = url + "&subCompanyId="+subCompanyId;
		...
		function editApp(appId){
			...
			appDlg.Width = 700;//定义长度
			appDlg.Height = 260;
			appDlg.URL = "/formmode/setup/addDefineApp.jsp?appId="+appId+"&flag=edit";
		...
		function preImportApp(appid){
			...
			appDlg.URL = "/formmode/setup/addDefineApp.jsp?appId="+appid+"&flag=import&isImportant=<%=formModeConfig.isImportchecksametable()%>";
			<%}else{%>
			appDlg.URL = "/formmode/setup/addDefineApp.jsp?appId="+appid+"&flag=import&isImportant=<%=formModeConfig.isImportchecksametable()%>";
			<%}%>
	...
	function selectOrg(){
		...
		dialog.Width = winWidth-25 ;
		dialog.Height = winHight+48;
...
<body id="mainBody" name="mainBody">
	<%if(fmdetachable.equals("1")){%>
	<div id="detachDiv" class="e8_left_top">
			<table class="e8_searchTable" style="width: 99%;">
				<colgroup>
					<col style="width: *" />
				...
				<tr>
					...
					<div style="width: 100%;background-color: white;line-height: 20px;">
					<div  id="subCompanyIdSpan" name="subCompanyIdSpan" style="color: gray;padding-left: 8px;overflow: hidden;height: 20px;width: 190px;line-height: 20px;">
						<%
						if(defaultSubCompanyId==-1){%>
					...
					<td width="20px" style="padding:2px;line-height:5px;cursor:pointer;" id="btnCreate" title="<%=SystemEnv.getHtmlLabelName(33256,user.getLanguage())%>"><!-- 选择分部 -->
					<img onclick="selectOrg();" style="width: 16px;" src="/formmode/images/a9_wev8.png" />
					</td>
				</tr>
			</table>
	</div>
