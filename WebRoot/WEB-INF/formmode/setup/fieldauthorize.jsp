		...
		<wea:item attributes="{'isTableList':'true'}">
			<table class=ListStyle id="oTable" cols=5  border=0 cellspacing=0>
				<tr class=header>
					...
					<td NOWRAP width='15%' style="display: none">权限项</td>
				...
				rs.executeSql("select * from workflow_billfield where billid="+formId+" and (detailtable is null or detailtable='') and fieldhtmltype=3 and type in(9,37,7,18,161,162,256,257) order by dsporder");
				int i=0;
				while(rs.next()){
					i++;
					...
					<td style="display: none">
						<select name="opttype" onchange="changeOpttype(this);" style="display: <%=!showlayout?"none":"" %>"  browserid="<%=fielddbtype%>">
							<option value="0" <%=opttype.equals("0")?"selected":"" %>>查看</option>
							<option value="2" <%=opttype.equals("2")?"selected":"" %>>编辑</option>
						</select>
					</td>
					<td>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(82047,user.getLanguage())+orderid+SystemEnv.getHtmlLabelName(33331,user.getLanguage()) %>'><!-- 子表{1}字段 -->
		<wea:item attributes="{'isTableList':'true'}">
			<table class=ListStyle id="oTable" cols=5  border=0 cellspacing=0>
				...
				rs.executeSql("select * from workflow_billfield where billid="+formId+" and detailtable='"+tablename+"' and fieldhtmltype=3 and type in(9,37,7,18,161,162,256,257) order by dsporder");
				while(rs.next()){
					int id = Util.getIntValue(rs.getString("id"),0);
					String fieldhtmltype = Util.null2String(rs.getString("fieldhtmltype"));
					String type = Util.null2String(rs.getString("type"));
