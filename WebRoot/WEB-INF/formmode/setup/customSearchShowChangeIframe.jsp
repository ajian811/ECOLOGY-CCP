...
<%
	...
	if(!StringHelper.isEmpty(value)){
		rs.execute("select * from customfieldshowchange where customid="+customid+" and fieldid="+fieldid +" order by id");
		while(rs.next()){
...
<body>
	...
	<div id="SeleteItem">
					...
					<div id="div5" style="vertical-align: middle; height:30px; line-height: 30px;text-align: left;padding-left: 5px;">
						<button type="button" class="addbtn" id="btnaddRow" name="btnaddRow" onclick='addoTableRow()' title="<%=SystemEnv.getHtmlLabelName(15443,user.getLanguage())%>" style="margin-bottom:-5px;"></BUTTON><!-- 添加内容 -->
						<button type="button" class="delbtn" id="btnsubmitClear" name="btnsubmitClear" onclick='submitClear()' title="<%=SystemEnv.getHtmlLabelName(15444,user.getLanguage())%>" style="margin-bottom:-5px;"></BUTTON><!-- 删除内容 -->
					</div>
					<%}%>
					<div id="div5_5">
	...
	<div style="padding: 10px;color: red;">
		<p>说明:</p>
		<p>1.字段条件支持$date$解析当前日期。</p>
		<p>2.字段条件支持GetDate(date,year,month,day)方法，此方法返回日期。比如GetDate(2015-06-17,,,1)表示返回日期为2015-06-18，
		其中year，month，day这几个参数，如果是正数则表示加相应的数字，如果是负数则表示减相应的数字</p>
		<p>3.GetDate(date,year,month,day)此方法中的四个参数都支持解析表单字段值，比如GetDate($objdate$,,,$zs$),
		其中objdate，zs为字段列名，这样会解析出字段的值然后返回一个日期。如需设置明细表字段时，对应字段名需要加上对应明细表表名，比如GetDate($uf_objinfo_dt1.objdate$,,,$zs$)</P>
		<p>4.GetDate(date,year,month,day)此方法也支持解析$date$，比如GetDate($date$,1,,)，
		这样解析出当前日期加一年获取到的日期</p>
	</div>
	<%} %>
