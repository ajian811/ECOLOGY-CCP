...
if(operatelevel>0){
	...
	//if(formId>0){
		if(modelId>0)
		{
			//if(isvirtualform!=1){//导出模块
				RCMenu += "{"+SystemEnv.getHtmlLabelName(17416,user.getLanguage())+SystemEnv.getHtmlLabelName(19049,user.getLanguage())+",javascript:exportMode("+modelId+"),_self} " ;
				RCMenuHeight += RCMenuHeightStep;
			//}//创建菜单新建
			...
			RCMenuHeight += RCMenuHeightStep ;//查看菜单地址新建
...
<tr>
	...
	<td class="e8_tblForm_field">
		<%if(operatelevel>0){ %>
		<button type="button" class="copybtn2" onclick="onShowModeBrowser('showhtmlid','showhtmlspan',0)" title=""></button><!-- 选择模板 -->
		...
		<%} %>
		<span id="showhtmlspan"><a href="#" onclick="openFullWindowHaveBar('/formmode/setup/LayoutEdit.jsp?type=0&modeId=<%=modelId%>&formId=<%=formId%>&Id=<%=showhtmlid%>')"><%=showhtmlname%></a></span>
...
<tr>
	...
	<%} %>
		<span id="addhtmlspan"><a href="#" onclick="openFullWindowHaveBar('/formmode/setup/LayoutEdit.jsp?type=1&modeId=<%=modelId%>&formId=<%=formId%>&Id=<%=addhtmlid%>')"><%=addhtmlname%></a></span>
...
<tr>
	...
	<%} %>
		<span id="edithtmlspan"><a href="#" onclick="openFullWindowHaveBar('/formmode/setup/LayoutEdit.jsp?type=2&modeId=<%=modelId%>&formId=<%=formId%>&Id=<%=edithtmlid%>')"><%=edithtmlname%></a></span>
...
<tr>
	...
	<%} %>
		<span id="monitorhtmlspan"><a href="#" onclick="openFullWindowHaveBar('/formmode/setup/LayoutEdit.jsp?type=3&modeId=<%=modelId%>&formId=<%=formId%>&Id=<%=monitorhtmlid%>')"><%=monitorhtmlname%></a></span>
...
<tr>
	...
	<%} %>
		<span id="printhtmlspan"><a href="#" onclick="openFullWindowHaveBar('/formmode/setup/LayoutEdit.jsp?type=4&modeId=<%=modelId%>&formId=<%=formId%>&Id=<%=printhtmlid%>')"><%=printhtmlname%></a></span>
...
<tr>
	...
	<td class="e8_tblForm_field">
		<select name="categorytype" id="categorytype" onchange="categorytypeChange(this)">
			<option value="0" <%=(0==categorytype?"selected":"") %>>固定目录</option>
			<option value="1" <%=(1==categorytype?"selected":"") %>>选择目录</option>
		</select>&nbsp;
        ...
        if (ajax.readyState == 4 && ajax.status == 200) {
            try{
            	if(ajax.responseText == "0"){
					...
					/* window.showModalDialog(url, '', "dialogWidth=600px;"); */
...
function onShowModeBrowser(ids,spans,type){
	urls = "/formmode/setup/FormModeHtmlBrowser.jsp?modeId="+<%=modelId%>+"&formId="+<%=formId%>;
...
function checkIsAllowReply(obj){
	if($(obj).attr("checked")){
		$(obj).val(1);
	}else{
		$(obj).val(0);
