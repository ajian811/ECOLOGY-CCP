...
if(action.equalsIgnoreCase("saveFormfield")){
	...
	for(int i = 0; i < dataArr.size(); i++){
		...
		sql += " select '"+fieldid+"' as fieldid,COUNT(1) as dcount from "+tablename+" where "+fieldname+"='"+changedValue+"' and id!="+dataId;
		if(i != (dataArr.size() - 1)){
			sql += " union all";
		}
	}
	...
	List<Map<String, Object>> resultList = baseDao.getResultByList(sql);
	...
	try{
		...
		errorMsg = URLEncoder.encode(ep.getMessage());
	}
	response.sendRedirect("/formmode/setup/formIndex.jsp?formId="+formId+"&errorMsg="+errorMsg);
