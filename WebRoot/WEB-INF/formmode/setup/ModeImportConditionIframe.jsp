...
public String getIdName(String id,int type){
    ...
    while(rs2.next()){
    	...
    	if(!detailtable.equals("")){
    		fieldname += "(明细"+detailtable.substring(detailtable.length()-1,detailtable.length())+")";
    	}
    	if( type == 0){
...
<%} %>
<form name="linkageattr" id="linkageattr" method="post" action="ExcleFieldOperation.jsp" >
						...
						<%
	  				       ...
	  				       <td class="e8_tblForm_field">
	  				           <%if(sourcetype == 1){ %>
	  				     	   ...
	  				     	   <%} %>
						            ...
						            <span ><%=getIdName(selectid,1) %></span>
	  				     ...
	  				     <TD class="e8_tblForm_field" >
	  				      <div STYLE="<%=css %>" id="valueTd_<%=i %>">
	  				         <%if(sourcetype == 1){ %>
	  				         <select name="validationName_<%=i %>" id="validationName_<%=i %>">
	  				        <%
	  				         if(!selectid.equals("")){
                            ...
                            <option value="<%=value %>" <%if(selectvalue.equals(value)){ %> selected <%} %>><%=label %></option>
	  				     	...
	  				     	<td class="e8_tblForm_field">
								...
								hasInput='' isSingle="true" hasBrowser = "true" isMustInput="2"  tempTitle="<%=tempTitle %>"
		...
		switch(j) {
            ...
            case 1:
            {
		        ...
		        $.ajax({
			         ...
			         type:"post",
			         dataType:"json",
			         data:{formid:'<%=formid%>',action:'getSelectId'},
            ...
            case 2:
            {
                ...
                var sHtml = "<div STYLE='DISPLAY:NONE;' id='valueTd_"+rowindex+"'>"+
	  				        "<select name='validationName_"+rowindex+"' id='validationName_"+rowindex+"'>"+	  				     	    	  				     	     
	  				     	"</select>"+
	  				        "</div>";              
                oDiv.innerHTML = sHtml
                oCell.appendChild(oDiv);
...
function showColDialog1(param){
   	...
   	var url = "/formmode/setup/ImportValidationField.jsp?"+param
   	dialog.URL ="/systeminfo/BrowserMain.jsp?1=1&url="+escape(url);
	dialog.show();
...
function selectValue(obj,num){
   ...
   if(selectName != ""){
      var url = "/formmode/setup/ExcleFieldOperation.jsp?action=getSelectInfo";
      $.post(
         url,
         {selectid:selectName},
         function(data){
...
function getBrowserUrl(num){   
     ...
     return  "/systeminfo/BrowserMain.jsp?url=/formmode/setup/ImportValidationField.jsp?modeid=<%=modeid%>&formid=<%=formid%>&id=<%=id%>&selectid="+selectName+"&fieldids="+fieldids;
}


function certain_clear(){
   window.top.Dialog.confirm("确定清空所有条件",function(){
