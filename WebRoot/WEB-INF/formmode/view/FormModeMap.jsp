...
//处理特殊值
String iframeurl = "/formmode/view/FormModeMapIframe.jsp?fieldid="+fieldid;
String tabname = SystemEnv.getHtmlLabelName(126782, user.getLanguage()) ;//获取地址
%>
<!DOCTYPE html>
<HEAD>
...
try{
	if(type==2){
		parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>").val(addressinfo);
		if(parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>span")){
			if(parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>span").attr("isedit")!=1){
				parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>span").html(addressinfo);
			}
		}
	}else{
		var temp = parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>").val();
		parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>").val(temp+addressinfo);
		if(parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>span")){
			if(parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>span").attr("isedit")!=1){
				parent.parent.jQuery("#tabcontentframe").contents().find("#field<%=fieldid%>span").html(temp+addressinfo);
			}
		}
	}
