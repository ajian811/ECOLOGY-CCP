...
<html>
  <head>
	...
	<style type="text/css">
	 ...
	 .fmComentClass{margin: 0px 0px 10px 0px; border: 1px solid #dadada;background: #fff;font-size: 12px;vertical-align: middle;padding: 0px 10px 0px 0px; clear: both;}
	 ...
	 .fmCommenttr{}
	 .fmtime{color: #929393;}
	 .fmCommentlist{border: 1px solid #dee3e3;padding:6px 8px 6px 8px;background-color:#f9f9f9;margin-top:5px;}
	...
	<script type="text/javascript">
		...
		function showReplay(comid,sign,commenttopid,commentusersid){
			...
			if(sign=="quotes"){
			   ...
			   jQuery("#tipcontent_"+comid).html("请填写回复内容");
			}else{
			   if(sign=="comment"){
			   ...
			   jQuery("#tipcontent_"+comid).html("请填写评论内容");
			}
			var replyFrameSrc = '<%=replyFrameSrc+"&billid=&type=1"%>';
			replyFrameSrc += "&iframeSign=replyFrame_"+comid+replyFrameParams;
		...
		function delComment(obj,commentid){
			 window.top.Dialog.confirm("确认删除评论？",function(){
		 ...
		 function deleteComment(obj,comid,sign){
		     window.top.Dialog.confirm("确认删除该评论记录？",function(){
		    	jQuery.post("/formmode/view/formModeComOperation.jsp?method=deleteComment",{"rqmodeid":<%=modeId%>,"rqid":<%=billId%>,"commentid":comid,"sign":sign},function(data){
		    		if(jQuery.trim(data)=="3"){
						window.top.Dialog.alert("评论时间已经超过10分钟！");
						return;
					}else if(jQuery.trim(data)=="1"){
						window.top.Dialog.alert("评论有新回复！");
						return;
					}
		    		if(sign=='comment'){
		    			if(jQuery.trim(data)=="2"){
							window.top.Dialog.alert("评论被引用！");
							return;
						}
		...
		function editComment(comid,quotesid){
			...
			jQuery.post("/formmode/view/formModeComOperation.jsp?method=editComment",{"rqmodeid":<%=modeId%>,"rqid":<%=billId%>,"commentid":comid},function(data){
				if(jQuery.trim(data)=="1"){
					window.top.Dialog.alert("评论有最新回复！");
					return;
		...
		function resetIframeHeight(iframeSign){
			...
			if (iframe) {
					...
					if($frmmain.length > 0){
						iframe.style.height = $frmmain.height() + "px";
					}
			}
		}
	</script>
	    	...
	    	<div class="signblockbarclass" style="width: 100%;">
		        <ul class="signblockbarULclass">
		            <li class="current" id="oTDtype_0"> 
		            	<a href="javascript:signtabchanges(0)">评论相关</a> 
		            </li>
		        </ul>
			    	    	...
			    	    	<TABLE style="height:100%;table-layout: fixed; " id="toolBarTbl"  cellpadding="0px" cellspacing="0px"  width="100%" align="right" >
		    	    			<tr align="left">
						           <td class="fmReplyDoOver" searchSignName="showSearch" onclick="toggleSearch(this)">
						        		...
						        		<span style="background:#eeeeee;color:#949494;font-size:14px;" class="fmTbItmOver" >搜索</span>   
						           </td>
					        	</tr>
			    			</TABLE>
						</td>
		    	 ...
		    	 <wea:layout type="4col">
		            ...
		            <wea:group context="">
		                <wea:item type="toolbar">
		                    ...
		                    <input class="e8_btn_submit" type="button" value="重置" onclick="resetSearch()" />
		                    <span class="e8_sep_line">|</span>
...
function showAllComment(sign,pagNo){
	...
	if(sign!=-1){
		...
		jQuery("#loading-msg").html("正在加载数据，请稍后...");
