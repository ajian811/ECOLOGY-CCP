...
2 selection process must be customized browsing(cptcapitalinfo) fields and number fields, the main table or a detailed table all can (asset purchase process is customized browsing(cptcapital) field);
...
配置說明:
1.選擇需要實現資産流轉功能的流程;
2.被選流程必須要有自定義單選(cptcapitalinfo)浏覽字段和數量字段,主表或明細表皆可(資産申購流程是自定義單選(cptcapital)浏覽字段);
...
配置说明:
1.选择需要实现资产流转功能的流程;
2.被选流程必须要有自定义单选(cptcapitalinfo)浏览字段和数量字段,主表或明细表皆可(资产申购流程是自定义单选(cptcapital)浏览字段);
