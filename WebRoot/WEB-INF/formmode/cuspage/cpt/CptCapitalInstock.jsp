...
<jsp:include page="/systeminfo/commonTabHead.jsp">
   ...
   <jsp:param name="navName" value='<%="批量验收" %>'/>
</jsp:include>
...
<wea:layout type="4col" attributes="{'expandAllGroup':'true'}">
	...
	<wea:group context="入库明细" attributes="" >
		...
		success: function do4Success(data){
				if(data.msg=="true"){
			       ...
			       $("#rightMenuIframe").hide();
					...
					window.top.Dialog.alert("入库单已被采购人收回！");
					return;
				}
			}
		});
				...
				if (numindex>0){
			 		for (var i=0;i<numindex;i++){
						if (document.all("node_"+i+"_stockindate").value.replace(/-/g,"")>dateIn){
							...
							window.top.Dialog.alert("入库日期不能小于购置日期!");
				    		return false;
						}
					}
				} 
...
function onReject(obj){
	window.top.Dialog.confirm('您确定将此入库单收回吗？',function(){
		obj.disabled = true;
       ...
       $("#rightMenuIframe").hide();
		jQuery.post(
			...
			{"method":"reject","id":'<%=id %>'},
			function(data){
				window.top.Dialog.alert("收回成功！",function(){
					try{
						parentWin._table.reLoad();
						updatecptstockinnum("<%=user.getUID() %>");
						window.location.href="/cpt/capital/CapitalBlankTab.jsp?title=修改入库单&url=/cpt/capital/CptCapitalInstock1tab.jsp?isdialog=1%26id=<%=id %>";
					}catch(e){}
				});
			}
		);
...
function onReApply(obj){
    ...
    $("#rightMenuIframe").hide();
	window.location.href="/cpt/capital/CapitalBlankTab.jsp?title=重新填写入库单&url=/cpt/capital/CptCapitalInstock1tab.jsp?isdialog=1%26id=<%=id %>";			
}
</script>
</BODY>
