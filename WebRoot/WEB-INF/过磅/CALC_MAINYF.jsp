...
<%
    ...
    try {
        ...
        if ("0".equals(lx)) {
            ...
            String sql = "select zyf,sfyg from formtable_main_45 where zxjhh = '" + zxjhh + "' and REQUESTID IS NULL";
            ...
            //    dtsql += " left join formtable_main_45_dt3 t2 on t1.id = t2.mainid";
            //    dtsql += " where t1.zxjhh = '" + zxjhh + "' AND t1.REQUESTID IS NULL";
		...
		*/
        ...
        //SO需要同时更新流程明细和建模明细
        if ("0".equals(lx)) {
            flag = updateMx(list, sfyg, dtname1, fieldname, dtname2) && updateMx(list2, sfyg, dtname1, fieldname, dtname2);
        }
        //PO只需要更新流程明细
        if ("1".equals(lx)){
...
%>
<%!public List<Map<String, Double>> getYfList(String lx,String zxjhh,Double total,String sfyg) {
    RecordSet dtrs=new RecordSet();
    BaseBean log=new BaseBean();
    List<Map<String, Double>> list = new ArrayList<Map<String, Double>>();
    Double dtlc=0.00;
    ...
    if ("0".equals(lx)){
        str0=" AND t1.REQUESTID IS not NULL";
    }
    if ("1".equals(sfyg)) {
        dtsql = "select t2.* from formtable_main_45 t1";
        dtsql += " left join formtable_main_45_dt3 t2 on t1.id = t2.mainid";
        dtsql += " where t1.zxjhh = '" + zxjhh + "'";
        dtsql+=str0;
        dtsql += " order by t2.id";
    ...
    if ("0".equals(sfyg)){
        dtsql = "select t3.* from formtable_main_45 t1";
        dtsql += " left join formtable_main_45_dt3 t3 on t1.id = t3.mainid";
        dtsql += " where t1.zxjhh = '" + zxjhh + "'";
        dtsql+=str0;
    }
