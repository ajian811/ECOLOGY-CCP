...
<%
    ...
    try {
        ...
        if ("0".equals(lx)) {
            ...
            String sql = "select zgz,bzclzzl,gbzl,sfyg,id from formtable_main_45 where zxjhh = '" + zxjhh + "' and requestid is null";
        ...
        for (int i = 0; i < list.size(); i++) {
             ...
             */
            Double jhlljz0=Double.parseDouble(list.get(i).get("jhlljz"));
            if (i==(list.size()-1)) {
                sjjz=calculateJZ(zhwjz,total2);
            }else {
                sjjz = mulTwo(divUnReserved(jhlljz0, total), zhwjz);
            }


            log.writeLog("jhlljz0:" + jhlljz0 + ",total:" + total + ",zhwjz:" + zhwjz+",total2:"+total2+",sjjz:"+sjjz);

            total2=add(total2,sjjz);
...
%>
<%!
    public Double divUnReserved(double v1, double v2) {
        //计算除法
        BigDecimal b1 = new BigDecimal(Double.valueOf(v1));
        BigDecimal b2 = new BigDecimal(Double.valueOf(v2));
        return b1.divide(b2, 6, BigDecimal.ROUND_HALF_UP).doubleValue();
...
%>
<%!public List<Map<String,String>> getJzList(String id) {
    List<Map<String, String>> list = new ArrayList<Map<String, String>>();
    BaseBean log = new BaseBean();
    RecordSet dtrs = new RecordSet();
    String dtid = "";
    Double jhlljz = 0.00;
    String dtSql = "select sl,jhyzl,jzl,id from formtable_main_45_dt3 where mainid = '" + id + "'";
    ...
    while (dtrs.next()) {
        ...
        jhlljz = mul(divUnReserved(Double.valueOf(jzl), Double.valueOf(sl)), Double.parseDouble(jhyzl));//(Double.parseDouble(jzl) / Double.parseDouble(sl)) * Double.parseDouble(jhyzl);//每条明细计划净重
        log.writeLog("计算前total："+total);
        total=add(jhlljz,total);
        map.put("jhlljz", "" + jhlljz);
        map.put("dtid", dtid);
        list.add(map);
