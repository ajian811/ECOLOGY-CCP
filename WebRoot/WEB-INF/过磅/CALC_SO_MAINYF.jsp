...
<%
	...
	*/
	...
	log.writeLog("调用CALC_SO_MAINYF.jsp开始");
			...
			*/
			...
			if (rs.execute(sql)) {
				if (rs.next()) {
					...
					gbzl = rs.getDouble("gbzl");//过磅重量
					jgd = rs.getDouble("jgd");//价格档（单价）
					lc = rs.getDouble("id");//里程
					tczyf = rs.getDouble("tczyf");//同城费用
					flxzj = rs.getDouble("flxzj");//辅线路费用			
					...
					if ("0".equals(jfms)) {//包车
						...
						if (rs1.next()) {
							if ("1".equals(sfyg)) {
								cuont = rs1.getDouble("d2sum");
								mainPrice = mul(cuont, jgd);
							} else if ("0".equals(sfyg)) {
								cuont = rs1.getDouble("d3sum");
								mainPrice = mul(cuont, jgd);
							} else {
								mainPrice = 0.00;
							}
			...
			if (total > 0 && "1".equals(sfyg)) {
				...
				if (dtrs.execute(dtsql)) {
					while (dtrs.next()) {
						...
						dtlc = dtrs.getDouble("lc");//里程
						sjftjz = dtrs.getDouble("sjftjz");//实际分摊净重
			...
			if (total > 0 && "1".equals(sfyg)) {
				...
				if (dtrs.execute(dtsql)) {
					while (dtrs.next()) {
						Map<String, Double> map = new HashMap<String, Double>();
						dtlc = dtrs.getDouble("lc");//里程
						sjftjz = dtrs.getDouble("ftzl");//实际分摊净重
						zqz += mul(dtlc, sjftjz);//总权重
						dtid = Double.parseDouble(dtrs.getString("id"));//明细数据Id
		...
		*/
		...
		if ("1".equals(sfyg)) {
			...
			for (Map<String, Double> m : list) {
				...
				boolean flag = uprs.execute(updateSql);
				if (!flag) {
			...
			for (Map<String, Double> m : list) {
				...
				if (uprs.executeSql(updateSql)) {
					log.writeLog("更新明细表2成功");
				} else {
					log.writeLog("执行明细表数据id" + String.valueOf(m.get("dtid")) + "错误");
					break;
				}
