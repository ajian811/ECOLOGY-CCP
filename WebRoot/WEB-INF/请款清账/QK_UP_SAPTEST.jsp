...
<%
    ...
    try {
        ...
        //while (rs.next()){
            ...
            //VATDATE=currdate;//Tax Reporting Date
            ...
            //REF_DOC_NO=Util.null2String(rs.getString("cz"));//参考凭证号
            //HEADER_TXT=Util.null2String(rs.getString("pztt"));//凭证抬头文本
            //PSTNG_PERIOD=Util.null2String(rs.getString("jzqj"));//记账期间
            ...
            headers.put("DOC_DATE","20180103");//凭证日期
            headers.put("PSTNG_DATE","20180128");//记帐日期
            headers.put("VATDATE","20180128");//Tax Reporting Date
            headers.put("DOC_TYPE","KR");//凭证类型
            headers.put("COMP_CODE","7020");//公司代码
            headers.put("CURRENCY","MYR");//货币代码
            headers.put("EXCHNG_RATE",EXCHNG_RATE);//汇率
            headers.put("REF_DOC_NO","TMSB/F/0063-18");//参考凭证号
            headers.put("HEADER_TXT","FORWARDING FEE");//凭证抬头文本
            headers.put("PSTNG_PERIOD","TX");//记账期间
            headers.put("USER_NAME","DIGI_OA");//用户名
        ...
        while (rs.next()){
            ...
            item.put("DOC_DATE",DOC_DATE);//凭证日期
            item.put("PSTNG_DATE",PSTNG_DATE);//记帐日期
            item.put("VATDATE",VATDATE);//Tax Reporting Date
            ...
            item.put("PAY_DATE",PAY_DATE);//基准日期
            ...
            item.put("PLAN_DATE",PLAN_DATE);//计划日期
        ...
        JCO.Table inTableParams1 = function.getTableParameterList().getTable("IF_DOC_ITEMS");
