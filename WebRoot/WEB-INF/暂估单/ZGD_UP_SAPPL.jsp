...
<%
    log.writeLog("进入暂估单批量上抛ZGD_UP_SAPPL.jsp");
    String billids = request.getParameter("billids");
    log.writeLog("billids:"+billids);
    String[] billisArray=billids.split(",");
    RecordSet rs=new RecordSet();
    List<String> djbhArray=new ArrayList<String>();
    for (int j = 0; j <billisArray.length ; j++) {
        String sql="SELECT DJBH,ZXPLANNO,LGBH FROM UF_ZGFY WHERE ID="+billisArray[j];
        rs.writeLog(sql);
        rs.execute(sql);
        String djbh="";
        while (rs.next()){
    ...
    JSONArray jsonArray=plSapUpload(djbhArray);
    out.clear();
...
<%! public JSONArray plSapUpload(List<String> djbhArray) {
    ...
    JSONObject objectresult = new JSONObject();
        ...
        String djbh="";
        String zxjhh="";
        flagfor:for (int j = 0; j <djbhArray.size() ; j++) {
            djbh = djbhArray.get(j);
            objectresult.put("djbh",djbh);
            ...
            if (rs.execute(sql)) {
                while (rs.next()) {
                    String djstatus=rs.getString("djstatus");
                    if ("1".equals(djstatus)){
                        objectresult.put("msg","It's already uploaded");
                        jsonArray.add(objectresult);
                        continue flagfor;
                    }
                    if (index == 0) {
                        ...
                        zxjhh = Util.null2String(rs.getString("ZXPLANNO"));
