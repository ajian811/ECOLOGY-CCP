...
<jsp:useBean id="ModeShareManager"
	class="weaver.formmode.view.ModeShareManager" scope="page" />
<jsp:useBean id="ModeRightInfo"
	class="weaver.formmode.setup.ModeRightInfo" scope="page" />
<jsp:useBean id="FormModeTransMethod"
	class="weaver.formmode.search.FormModeTransMethod" scope="page" />
<jsp:useBean id="DeleteData"
	class="weaver.formmode.search.batchoperate.DeleteData" scope="page" />
<jsp:useBean id="FormModeRightInfo"
	class="weaver.formmode.search.FormModeRightInfo" scope="page" />
<jsp:useBean id="CustomSearchService"
	class="weaver.formmode.service.CustomSearchService" scope="page" />
<jsp:useBean id="FormModeConfig" class="weaver.formmode.FormModeConfig"
	scope="page" />
<jsp:useBean id="xssUtil" class="weaver.filter.XssUtil" />
<jsp:useBean id="ExpandBaseRightInfo"
	class="weaver.formmode.setup.ExpandBaseRightInfo" scope="page" />
...
<LINK href="/js/jquery/plugins/multiselect/jquery.multiselect_wev8.css"
	type=text/css rel=STYLESHEET>
<link href="/js/jquery/plugins/multiselect/style_wev8.css" type=text/css
	rel=STYLESHEET>
<link
	href="/formmode/js/jquery/jquery-ui-1.10.3/themes/base/jquery-ui_wev8.css"
	type=text/css rel=STYLESHEET>
<script type="text/javascript"
	src="/formmode/js/jquery/jquery-ui-1.10.3/ui/minified/jquery-ui.min_wev8.js"></script>
<script language="javascript"
	src="/js/jquery/plugins/multiselect/jquery.multiselect.min_wev8.js"></script>
<script language="javascript"
	src="/js/jquery-autocomplete/jquery.autocomplete_wev8.js"></script>
<script language="javascript"
	src="/formmode/js/customSearchOperate_wev8.js?v=7"></script>
<style>
.ui-multiselect-menu {
	z-index: 9999999;
}
...
%>
	<input class="e8_btn_top_first" value="费用重算"
		onclick="javascript:refreshZGD();"
		style="max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; float: right;"
		title="费用重算" type="button">
	&nbsp;
	<input class="e8_btn_top_first" value="SAPUPLOAD"
		   onclick="javascript:sapUpLoadPL();"
		   style="max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; float: right;"
		   title="SAPUPLOAD" type="button">
	<table id="topTitle" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td class="rightSearchSpan" style="text-align: right;">
				...
				value="<%=SystemEnv.getHtmlLabelName(82,user.getLanguage()) %>"
				onclick="javascript:Add()" /> <%  }
                		...
                		for (int i = 0 ; i < expandbuttonList.size() ; i++) {
                			...
                			if (!methodStr.isEmpty()) {%> <input type="button"
				class="e8_btn_top" value="<%=createname %>"
				onclick="javascript:<%=methodStr %>" /> <%      	} 
                		}
				  }
				%> <%if(searchKeyNum==0){%> <input type="hidden" id="searchName"
				name="searchName" /> <%}else{%> <input type="text" class="searchInput"
				id="searchName" name="searchName" value="<%=searchkeyname%>"
				title="<%=quickTitle %>" /> <%}%><!-- 高级搜索 --> <span
				id="advancedSearch" class="advancedSearch"><%=SystemEnv.getHtmlLabelName(21995,user.getLanguage())%></span>
				<span
	...
	<div class="advancedSearchDiv" id="advancedSearchDiv"
		style="display: hide; overflow: auto">
		<form id="frmmain" name="frmmain" method="post"
			action="/formmode/search/CustomSearchBySimpleIframeRefresh.jsp?<%=tempquerystring%>">
			<iframe id="selectChange" frameborder=0 scrolling=no src=""
				style="display: none"></iframe>
			<input id="customid" name=customid type=hidden value="<%=customid%>" />
			<input name=viewtype type=hidden value="<%=viewtype%>" /> <input
				name=issimple type=hidden value="<%=issimple%>" /> <input
				name=templateid id="templateid" type=hidden value="<%=templateid%>" />
			<input name="deletebillid" id="deletebillid" type=hidden value="" />
			<input name="method" id="method" type=hidden value="" /> <input
				name="searchMethod" id="searchMethod" type=hidden value="" /> <input
				type=hidden name="pageexpandid" id="pageexpandid" value="">
			<input name="treesqlwhere" id="treesqlwhere" type=hidden value="" />
			<input name="treesqlwhere1" id="treesqlwhere1" type=hidden
				value="<%=treesqlwhere1 %>" /> <input name="formmodeid"
				id="formmodeid" type="hidden" value="<%=modeid %>" /> <input
				type=hidden name=formid id="formid" value="<%=formID %>" /> <input
				type=hidden name=groupby id="groupby" value="<%=groupby %>" /> <input
				type=hidden name=searchkeyname id="searchkeyname"
				value="<%=searchkeyname%>" /> <input type=hidden
				name="isusersearchkey" id="isusersearchkey" value="0" /> <input
				type=hidden name="treenodeid" id="treenodeid"
				value="<%=treenodeid %>" /> <input type=hidden name="treeconid"
				id="treeconid" value="<%=treeconid %>" /> <input type=hidden
				name="<%="treecon"+treeconid+"_value" %>"
				id="<%="treecon"+treeconid+"_value" %>" value="<%=treeconvalue %>" />
			<wea:layout type="4col">
				<wea:group
					context='<%=SystemEnv.getHtmlLabelName(20331,user.getLanguage()) %>'
					attributes="">
					<!-- 自定义查询条件 -->
					<wea:item type="groupHead">
						<div style="margin-top: -2px; width: 219px;" class="templatecls">
							<select name="template" id="template"
								onChange="onChangeTemplate(this);" style="width: 135px;">
		    		...
		    		%>
							</select> <a href="javascript:void(0)" onclick="templateManage(2)"><%=SystemEnv.getHtmlLabelName(17857,user.getLanguage()) %><!-- 模板管理 --></a>
						</div>
					</wea:item>
					...
					<wea:item>
						<input type="checkbox" id="thisdate" name="thisdate" value="1"
							onclick="clickThisDate(this)"
							<%if(thisdate.equals("1")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("1".equals(thisdate)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchDate('1')">[<%=SystemEnv.getHtmlLabelName(15539,user.getLanguage())%>]
						</span>
						<!-- 本周 -->
						<input type="checkbox" id="thisdate" name="thisdate" value="2"
							onclick="clickThisDate(this)"
							<%if(thisdate.equals("2")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("2".equals(thisdate)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchDate('2')">[<%=SystemEnv.getHtmlLabelName(15541,user.getLanguage())%>]
						</span>
						<!-- 本月 -->
						<br />
						<input type="checkbox" id="thisdate" name="thisdate" value="3"
							onclick="clickThisDate(this)"
							<%if(thisdate.equals("3")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("3".equals(thisdate)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchDate('3')">[<%=SystemEnv.getHtmlLabelName(21904,user.getLanguage())%>]
						</span>
						<!-- 本季 -->
						<input type="checkbox" id="thisdate" name="thisdate" value="4"
							onclick="clickThisDate(this)"
							<%if(thisdate.equals("4")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("4".equals(thisdate)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchDate('4')">[<%=SystemEnv.getHtmlLabelName(15384,user.getLanguage())%>]
						</span>
						<!-- 本年 -->
					</wea:item>

					<wea:item><%=SystemEnv.getHtmlLabelName(81448,user.getLanguage())%></wea:item>
					<!-- 创建人属于 -->
					<wea:item>
						<input type="checkbox" id="thisorg" name="thisorg" value="1"
							onclick="clickThisOrg(this)"
							<%if(thisorg.equals("1")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("1".equals(thisorg)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchOrg('1')">[<%=SystemEnv.getHtmlLabelName(21837,user.getLanguage())%>]
						</span>
						<!-- 本部门 -->
						<input type="checkbox" id="thisorg" name="thisorg" value="2"
							onclick="clickThisOrg(this)"
							<%if(thisorg.equals("2")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("2".equals(thisorg)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchOrg('2')">[<%=SystemEnv.getHtmlLabelName(81362,user.getLanguage())%>]
						</span>
						<!-- 本部门(包含下级部门) -->
						<br />
						<input type="checkbox" id="thisorg" name="thisorg" value="3"
							onclick="clickThisOrg(this)"
							<%if(thisorg.equals("3")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("3".equals(thisorg)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchOrg('3')">[<%=SystemEnv.getHtmlLabelName(30792,user.getLanguage())%>]
						</span>
						<!-- 本分部 -->
						<input type="checkbox" id="thisorg" name="thisorg" value="4"
							onclick="clickThisOrg(this)"
							<%if(thisorg.equals("4")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("4".equals(thisorg)){%>#0000FF<%}else{%>#6A9EE6<%}%>;cursor:hand"
							onclick="quickSearchOrg('4')">[<%=SystemEnv.getHtmlLabelName(81363,user.getLanguage())%>]
						</span>
						<!-- 本分部(包含下级分部) -->
					</wea:item>
					<%}%>
					<%if(isEnabled&&viewtype!=3&&!isVirtualForm){%>
					<wea:item><%=SystemEnv.getHtmlLabelName(82439,user.getLanguage())%></wea:item>
					<!-- 数据状态 -->
					<wea:item>
						<input type="checkbox" id="enabled" name="enabled" value="1"
							onclick="clickEnabled(this)"
							<%if(enabled.equals("1")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("1".equals(enabled)){%>#0000FF<%}else{%>#6A9EE6<%}%>"><%=SystemEnv.getHtmlLabelName(25426,user.getLanguage())%></span>
						<!-- 未读 -->
						<input type="checkbox" id="enabled" name="enabled" value="2"
							onclick="clickEnabled(this)"
							<%if(enabled.equals("2")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("2".equals(enabled)){%>#0000FF<%}else{%>#6A9EE6<%}%>"><%=SystemEnv.getHtmlLabelName(21950,user.getLanguage())%></span>
						<!-- 反馈 -->
						<input type="checkbox" id="enabled" name="enabled" value="3"
							onclick="clickEnabled(this)"
							<%if(enabled.equals("3")){out.println("checked");}%>>
						<span
							style="font-size:12px;TEXT-DECORATION:none;color:<%if("3".equals(enabled)){%>#0000FF<%}else{%>#6A9EE6<%}%>"><%=SystemEnv.getHtmlLabelName(25425,user.getLanguage())%></span>
						<!-- 已读-->
					</wea:item>
					<%}%>
...
%>
					<wea:item>
						<!-- 是否作为查询条件 -->
						<input type='checkbox' name='check_con'
							title="<%=SystemEnv.getHtmlLabelName(20778,user.getLanguage())%>"
							value="<%=id%>" style="display: none" <%=checkstr%>>
						<%=label%>
						<input type=hidden name="con<%=id%>_htmltype"
							value="<%=htmltype%>">
						<input type=hidden name="con<%=id%>_type" value="<%=type%>">
						<input type=hidden name="con<%=id%>_colname" value="<%=name%>">
						<input type=hidden name="con<%=id%>_viewtype"
							value="<%=field_viewtype%>">
					</wea:item>
					<wea:item>
						<%
...
%>
						<select class=inputstyle name="con<%=id%>_opt"
							style="<%=display%>width:90">
							...
							<!--不等于-->
							<%}%>
							<option value="3" <%if(tmpopt==3){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(346,user.getLanguage())%></option>
							<!--包含-->
							<option value="4" <%if(tmpopt==4){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15507,user.getLanguage())%></option>
							<!--不包含-->

						</select>
						<input type=text class=InputStyle style="width: 50%"
							name="con<%=id%>_value" value="<%=tmpvalue%>">

						<SPAN id=remind style='cursor: hand'
							title='<%=SystemEnv.getHtmlLabelName(82346,user.getLanguage())%>&#10;<%=SystemEnv.getHtmlLabelName(82347,user.getLanguage())%>&#10;<%=SystemEnv.getHtmlLabelName(82348,user.getLanguage())%>&#10;<%=SystemEnv.getHtmlLabelName(82349,user.getLanguage())%>'>
...
%>
						...
						<%}%>
						<input type=text class=InputStyle size=10 name="con<%=id%>_value"
							onblur="checknumber('con<%=id%>_value');" value="<%=tmpvalue%>"
							style="width: 75px;">
						<select class=inputstyle name="con<%=id%>_opt1"
							style="<%=display%>width:90">
						...
						<%}%>
						<input type=text class=InputStyle size=10 name="con<%=id%>_value1"
							onblur="checknumber('con<%=id%>_value1');" value="<%=tmpvalue1%>"
							style="width: 75px;">
						<%
//=========================================================================================check类型
}
...
%>
						<input type="hidden" name="multiselectValue_con<%=id%>_value"
							id="multiselectValue_con<%=id%>_value"
							value="<%=multiselectvalue %>" />
						<select notBeauty=true class=inputstyle <%=multiselect %>
							value="<%=multiselectvalue %>" name="con<%=id%>_value"
							id="con<%=id%>_value"
							onchange="<%=selectchange%>;change_multiselectValue_con_value('<%=id%>',this.value)">
							<%
if(conditionTransition!=1){
 %>
							<option value=""></option>
...
%>
							<option value="<%=tmpselectvalue%>"
								<%if (tmpvalue.equals(""+tmpselectvalue)) {%> selected <%}%>><%=Util.toScreen(tmpselectname,user.getLanguage())%></option>
							<%} %>
						</select>
...
%>
						...
						<brow:browser viewType="0" name='<%=namestr%>'
							browserValue='<%=tmpvalue %>' _callback="change_con_name"
							_callbackParams="<%=id%>" browserUrl='<%= browserUrl %>'
...
%>
						<select class=inputstyle name="con<%=id%>_opt"
							style="<%=display%>width:90">
							<option value="5" <%if(tmpopt==5){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(18987,user.getLanguage())%></option>
							<!-- 被包含于 -->
							<option value="2" <%if(tmpopt==2){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(18988,user.getLanguage())%></option>
							<!-- 不被包含于 -->
						</select>
...
//=========================================================================================	
	...
	if(type.equals("2")){ //日期
		...
		%>
						<select name="datetype_<%=id%>_opt" id="datetype_<%=id%>_opt"
							style="display: block;"
							onchange="changeDateType(this,'datetype_<%=id%>_opt_span','con<%=id%>_value','con<%=id%>_valuespan','con<%=id%>_value1','con<%=id%>_value1span')">
							<option value="1" <%if(datetype_opt==1){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15537,user.getLanguage())%></option>
							<!-- 今天 -->
							<option value="2" <%if(datetype_opt==2){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15539,user.getLanguage())%></option>
							<!-- 本周 -->
							<option value="3" <%if(datetype_opt==3){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15541,user.getLanguage())%></option>
							<!-- 本月 -->
							<option value="7" <%if(datetype_opt==7){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(27347,user.getLanguage())%></option>
							<!-- 上个月 -->
							<option value="4" <%if(datetype_opt==4){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(21904,user.getLanguage())%></option>
							<!-- 本季 -->
							<option value="5" <%if(datetype_opt==5){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15384,user.getLanguage())%></option>
							<!-- 本年 -->
							<option value="8" <%if(datetype_opt==8){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(31276,user.getLanguage())+SystemEnv.getHtmlLabelName(25201,user.getLanguage())%></option>
							<!-- 上一年 -->
							<option value="6" <%if(datetype_opt==6){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(32530,user.getLanguage())%></option>
							<!-- 指定日期范围 -->
						</select>
						<span name="datetype_<%=id%>_opt_span"
							id="datetype_<%=id%>_opt_span"
							style="<%=datetype_opt_span_display%>"> <select
							class=inputstyle name="con<%=id%>_opt"
							style="<%=display%>width:90">
								<option value="1" <%if(tmpopt==1){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15508,user.getLanguage())%></option>
								<!-- 大于 -->
								<option value="2" <%if(tmpopt==2){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(325,user.getLanguage())%></option>
								...
								<option value="6" <%if(tmpopt==6){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15506,user.getLanguage())%></option>
								<!-- 不等于 -->
						</select> <%if(issimple){%><%=SystemEnv.getHtmlLabelName(348,user.getLanguage())%>
							<%}%><!-- 从 -->
							<button type=button class="<%=classStr %>"
								onclick="onSearchWFQTDate(con<%=id%>_valuespan,con<%=id%>_value,con<%=id%>_value1)"></button>
							<input type=hidden name="con<%=id%>_value" id="con<%=id%>_value"
							value="<%=tmpvalue%>"> <span name="con<%=id%>_valuespan"
							id="con<%=id%>_valuespan"><%=tmpvalue%></span> <select
							class=inputstyle name="con<%=id%>_opt1"
							style="<%=display%>width:90">
								<option value="1" <%if(tmpopt1==1){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15508,user.getLanguage())%></option>
								<!-- 大于 -->
								<option value="2" <%if(tmpopt1==2){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(325,user.getLanguage())%></option>
								...
								<option value="6" <%if(tmpopt1==6){%> selected <%}%>><%=SystemEnv.getHtmlLabelName(15506,user.getLanguage())%></option>
								<!-- 不等于 -->
						</select> <%if(issimple){%><%=SystemEnv.getHtmlLabelName(349,user.getLanguage())%>
							<%}%><!-- 到 -->
							<button type=button class="<%=classStr %>"
								onclick="onSearchWFQTDate(con<%=id%>_value1span,con<%=id%>_value1,con<%=id%>_value)"></button>
							<input type=hidden name="con<%=id%>_value1"
							id="con<%=id%>_value1" value="<%=tmpvalue1%>"> <span
							name="con<%=id%>_value1span" id="con<%=id%>_value1span"><%=tmpvalue1%></span>
						</span>
						<%
	}else{ //时间
		if(isresearch==1) {
		...
		%>
						...
						<%}%><!-- 从 -->
						<button type=button class="<%=classStr %>"
							onclick="onSearchWFQTTime(con<%=id%>_valuespan,con<%=id%>_value,con<%=id%>_value1)"></button>
						<input type=hidden name="con<%=id%>_value" id="con<%=id%>_value"
							value="<%=tmpvalue%>">
						...
						<%}%><!-- 到 -->
						<button type=button class="<%=classStr %>"
							onclick="onSearchWFQTTime(con<%=id%>_value1span,con<%=id%>_value1,con<%=id%>_value)"></button>
						<input type=hidden name="con<%=id%>_value1" id="con<%=id%>_value1"
							value="<%=tmpvalue1%>">
...
%>
						...
						<brow:browser viewType="0" name='<%=namestr%>'
							...
							isMustInput='1' completeUrl='<%=completeUrl%>' width="135px"
							browserSpanValue='<%=tmpname%>'>
						</brow:browser>
						<input type=hidden name="con<%=id%>_name" value='<%=tmpname%>'>
						<%
...
//=========================================================================================	
	...
	%>
						...
						<brow:browser viewType="0" name='<%=namestr%>'
							browserValue='<%=tmpvalue %>' _callback="change_con_name"
							_callbackParams="<%=id%>" browserOnClick='<%=browserOnClick%>'
							hasInput="true" isSingle="true" hasBrowser="true" isMustInput='1'
							completeUrl='<%=completeUrl%>' width="135px"
							browserSpanValue='<%=tmpname%>'>
						</brow:browser>
...
%>
						<brow:browser viewType="0" name='<%=namestr%>'
							browserValue='<%=tmpvalue %>'
							browserOnClick='<%=browserOnClick%>' hasInput="true"
...
%>
						...
						<brow:browser viewType="0" name='<%=namestr%>'
							browserValue='<%=tmpvalue %>' _callback="change_con_name"
							_callbackParams="<%=id%>" browserOnClick='<%=browserOnClick %>'
							browserUrl='<%=urls%>' onPropertyChange='<%=method2%>'
							hasInput="true" isSingle='<%=isSignle%>' hasBrowser="true"
							isMustInput='1' completeUrl='<%=completeUrl%>' width='<%=width%>'
							browserSpanValue='<%=tmpname%>'>
						</brow:browser>
...
%>
						...
						<brow:browser viewType="0" name="<%=namestr%>"
							browserValue="<%=tmpvalue %>" _callback="change_con_name"
							_callbackParams="<%=id%>" browserOnClick='<%=browserOnClick %>'
							browserUrl='<%=urls%>' nameSplitFlag="&nbsp" hasInput="true"
							isSingle="<%=isSingle%>" hasBrowser="true" isMustInput='1'
							completeUrl="<%=completeUrl%>" width="135px"
							browserSpanValue="<%=tmpname%>">
						</brow:browser>
						<input type=hidden name="con<%=id%>_name" value=''>
...
%>
						...
						<brow:browser viewType="0" name='<%=namestr%>'
							browserValue='<%=tmpvalue %>' _callback="change_con_name"
							_callbackParams="<%=id%>" browserUrl='<%= urls %>'
							hasInput="true"
							isSingle='<%=type.equals("194")?"false":"true" %>'
							hasBrowser="true" isMustInput='1' completeUrl='<%=completeUrl%>'
							width="135px" browserSpanValue='<%=tmpname%>'>
						</brow:browser>
						<input type=hidden name="con<%=id%>_name"
							class="<%=sapbrowser_name %>" cid='<%=id %>' value='<%=tmpname%>'>
						<%
//=========================================================================================	
...
%>
    ...
    %>
						<select initvalue="<%=tmpvalue %>" childsel="<%=linkfield %>"
							notBeauty=true class=inputstyle style="width: 175px;"
							name="con<%=id%>_value" id="con<%=id%>_value"
		...
		%>
				...
				<wea:group context="">
					<wea:item type="toolbar">
						<!-- 搜索 -->
						<input type="submit" class="e8_btn_submit"
							value="<%=SystemEnv.getHtmlLabelName(197,user.getLanguage())%>"
							id="searchBtn" onclick="setSearchName(parent)" />
						<!-- 重置 -->
						<input type="button"
							value="<%=SystemEnv.getHtmlLabelName(2022,user.getLanguage())%>"
							class="e8_btn_cancel"
							onclick="clearSearchName();resetSearch();resetMultiselect();resetDate();" />
						<% if (templateid > 0) { %>
						<!-- 保存 -->
						<input type="button"
							value="<%=SystemEnv.getHtmlLabelName(86,user.getLanguage())%>"
							" class="e8_btn_cancel" onclick="onSaveTempalte2();" />
						<% } else { %>
						<!-- 存为模板 -->
						<input type="button"
							value="<%=SystemEnv.getHtmlLabelName(18418,user.getLanguage())%>"
							" class="e8_btn_cancel" onclick="onSaveTempalte();" />
						<% } %>
						<!-- 取消 -->
						<input type="button"
			...
			<iframe id="selectChange_<%=iframeList.get(i) %>" frameborder=0
				scrolling=no src="" style="display: none"></iframe>
			<%} %>
			<%for(int i=0;i<ldselectfieldid.size();i++){%>
			<iframe id="selectChange_<%=ldselectfieldid.get(i) %>" frameborder=0
				scrolling=no src="" style="display: none"></iframe>
			<%} %>
		</form>
	</div>
	...
	<div id="splitPageContiner">
		<input type="hidden" name="pageWidthUnit" id="pageWidthUnit"
			value="px" /> <input type="hidden" name="pageWidth" id="pageWidth"
			value="" /> <input type="hidden" name="pageId" id="pageId"
			value="mode_customsearch:<%=customid %>" _showCol="false" />
		<%
String tableString = "";
...
%>
		<form id="batcheditForm" name="batcheditForm" method="post"
			action="/formmode/setup/customSearchActionForFront.jsp"
			target="batcheditIframe">
			<input type="hidden" id="action" name="action" value="batchEdit" />
			<input type="hidden" id="batcheditCustomid" name="batcheditCustomid"
				value="<%=customid%>" /> <input type="hidden" id="tableMax"
				name="tableMax" value="0" />
			<textarea id="modifiedRows" name="modifiedRows"
				style="display: none;"></textarea>
			<wea:SplitPageTag tableString='<%=tableString%>' mode="run" />
		</form>
		<iframe id="batcheditIframe" name="batcheditIframe"
			style="display: none;"></iframe>
	</div>
	<!-- 显示查询结果 -->
	...
	<script language=javascript
		src="/wui/common/jquery/plugin/Listener_wev8.js"></script>
	<script language=javascript
		src="/formmode/js/jquery/aop/jquery.aop.min_wev8.js"></script>
	<script>
...
function resetSplitPageWidth(){
	...
	if(<%=sumColWidth%> > 100 ) {
		var newwidth = Math.round(<%=allSumWidthFix%>*width/100);
		if(newwidth > width){
        ...
        success: function(data) {
            ...
            if (data) {           	
            	for(var i = 0;i<JSON.parse(data).result.length;i++){
            		Dialog.alert("装卸计划号:" + JSON.parse(data).result[i].zxplanno+",返回信息：" +JSON.parse(data).result[i].msg);  	
            	}
            }
            _table.reLoad();
        }
...
function sapUpLoadPL(){
    ...
    jQuery.ajax({
        url: "/weightJsp/ZGD_UP_SAPPL.jsp",
        data: {
            "billids":billids,
        },
        type: "POST",
        ...
        success: function(data) {
            ...
            if (data) {
                ...
                for(var i = 0;i<jsonDate.length;i++){
                    // Dialog.alert("装卸计划号:" + JSON.parse(data)[i].zxplanno+",返回信息：" +JSON.parse(data).result[i].msg);
               	var djbh=jsonDate[i].djbh;
               	var flag=jsonDate[i].flag;
               	var acdocno=jsonDate[i].acdocno;
               	var msg=jsonDate[i].msg;
				result+=djbh+":";
				if(flag=="X"){
				    result+="success,凭证号："+acdocno;
				}else{
				    result+="failure,err_msg:"+msg;
				}
				result+="\n";
                }
                Dialog.alert(result);
...
function modeopenFullWindowHaveBar(url,obj){
	...
	url = url +"&customTreeDataId=<%=customTreeDataId %>";
...
function onChangeTemplate(obj) {
	document.frmmain.action = "/formmode/search/CustomSearchBySimpleIframeRefresh.jsp?<%=tempquerystring%>";
...
<%}%>
	...
	<%
		String tempsqlquery = "select * from mode_CustomDspField where ismaplocation=1 and isshow=1 and customid="
				+ customid;
		rs.executeSql(tempsqlquery);
		if (rs.next()) {
	%>
	<script type="text/javascript"
		src="http://api.map.baidu.com/api?v=<%=Prop.getPropValue("map", "baidumapversion")%>&ak=<%=Prop.getPropValue("map", "baidumapak")%>"></script>
	<%
		}
	%>
...
<script language=javascript
	src="/js/ecology8/request/e8.browser_wev8.js"></script>
</html>
